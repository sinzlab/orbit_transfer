...
class EquivarianceTransfer(RepresentationRegularization):
    ...
    def enforce_proper_transform(self, batch_size):
        ...
        reg = 100 * torch.max(torch.zeros_like(volume), 0.85 - volume).sum()
        reg += 100 * torch.max(torch.zeros_like(volume), volume - 1.15).sum()
        shift = kernels[:, :, 3]
        reg += 100 * torch.max(torch.zeros_like(shift), torch.abs(shift) - 0.2).sum()
        self.tracker.log_objective(
